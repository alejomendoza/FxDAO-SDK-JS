import {
  ...
  VaultsContract,
  type DefaultContractParams,
} from '../index';
import * as stellarSDK from '@stellar/stellar-sdk';

type NetworkName = 'PUBLIC' | 'TESTNET';
...
function parseDenomination(value: string): Denomination {
  const upper = value.toUpperCase();
  if (!(upper in Denomination)) {
    throw new Error(`Invalid DENOMINATION "${value}". Use one of: ${Object.keys(Denomination).join(', ')}`);
  }
  return Denomination[upper as keyof typeof Denomination];
}

function parseNetwork(value: string): { networkName: NetworkName; passphrase: stellarSDK.Networks } {
  const upper = value.toUpperCase() as NetworkName;
  if (upper === 'PUBLIC') return { networkName: upper, passphrase: stellarSDK.Networks.PUBLIC };
  if (upper === 'TESTNET') return { networkName: upper, passphrase: stellarSDK.Networks.TESTNET };
  throw new Error(`Invalid NETWORK "${value}". Use "PUBLIC" or "TESTNET".`);
}

async function waitForResult(server: stellarSDK.rpc.Server, hash: string): Promise<void> {
  for (let i = 0; i < 30; i += 1) {
    await new Promise(resolve => setTimeout(resolve, 1500));
    const txResult = await server.getTransaction(hash);

    if (txResult.status === stellarSDK.rpc.Api.GetTransactionStatus.SUCCESS) return;
    ...
    if (txResult.status === stellarSDK.rpc.Api.GetTransactionStatus.NOT_FOUND) continue;
  }
...
async function main(): Promise<void> {
  ...
  const amount = withdrawXlm * 10_000_000n;

  const server = new stellarSDK.rpc.Server(rpcUrl);
  const keypair = stellarSDK.Keypair.fromSecret(secret);
  const source = keypair.publicKey();

  const globalParams: DefaultContractParams = {
    stellarSDK: {
